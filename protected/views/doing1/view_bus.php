...
<?php if ($model->accept->status == 2) { ?>
    <div class="modal-footer">
        ...
        echo CHtml::button('อนุมัติ', array(
            'onClick' => 'if(confirm("อนุมัติ?")){window.location="' . CHtml::normalizeUrl(array('/doing2/accept', 'id' => $model->paper_approval_bus_id)) . '"}',
            'class' => 'btn btn-success btn-sm',
        ));
        echo CHtml::button('ไม่อนุมัติ', array(
            'onClick' => 'if(confirm("ไม่อนุมัติ?")){window.location="' . CHtml::normalizeUrl(array('/doing2/notAccept', 'id' => $model->paper_approval_bus_id)) . '"}',
